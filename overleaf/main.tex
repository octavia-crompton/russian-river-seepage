...
\begin{equation}
\label{eq:dVdt}
    \frac{dV}{ dt} = Q_r + P\,A_e(h) + O + Q_{\mathrm{gw}} - \big(S + E\,A_e(h) + Q_{\mathrm{ov}}\big)
\end{equation}
...
$E$  the evaporation rate [L/T],
$Q_{\mathrm{gw}}$ net groundwater exchange (positive into the estuary) [L$^3$/T], and
$Q_{\mathrm{ov}}$ overflow across the berm [L$^3$/T]
...
As the estuary water level rises, the hydraulic gradient increases, and one may also expect $A_b$ or $L_b$ to vary, yielding a nonlinear dependence of $S$ on estuary water level  $ h$. To make any such dependencies explicit, $A_b$ and $L_b$ are expressed as $A_b = A'_b  h^x$ and $L_b = L'_b  h^y$, where $A'_b$ and $L'_b$ are constants and the exponents $x$ and $y$ account for any potential dependencies of $A_b$ and $L_b$ on $ h$.
...
\noindent where $C =  K A'_b / L'_b$.

Because mouth-scale geometry is unavailable and the analysis is restricted to closed-mouth periods, we do not model the berm dimensions $A_b$ and $L_b$ explicitly. Instead, we treat the geometric factor $A_b/L_b$ in Equation~\ref{eq:tildeK} as an empirical function of estuary stage and test whether the visitor-center water level $h_V$ serves as a proxy for it, comparing a constant-conductance (linear) model with an offset-power model in which the conductance varies with stage.

Combining the exponents in Equation~\ref{eq:Darcy_w_exponents} gives a geometric modulation $h^{\,x-y}$ referenced to estuary \emph{depth}, whereas the gauge reports $h_V$ relative to the NAVD88 datum ($h_V = 0$ near mean low tide). Because the active seepage cross-section need not vanish at $h_V = 0$, we introduce an additive offset $\delta_V$ so that $(h_V + \delta_V)$ approximates the depth-like quantity governing $A_b/L_b$. Normalizing by the median, $\zeta_V = (h_V + \delta_V)/m_V$, keeps the term dimensionless, so that $C_V$ retains units of conductance and equals the effective conductance at median stage ($\zeta_V = 1$); the exponent $b_V$ plays the role of $(x-y)$, with $b_V > 0$ corresponding to a seepage cross-section that grows and/or a flow path that shortens as the estuary fills.

Because $h_V$ and $\Delta h$ share the estuary-stage component, the conductance modulator $\zeta_V(h_V)$ and the driving head $\Delta h$ are not fully independent; the ocean term in $\Delta h$ contributes mainly tidal variance.
...
\begin{itemize}
  ...
  \item $C_V$ : effective conductance at median stage ($\zeta_V = 1$) [$\mathrm{L}^2\,\mathrm{T}^{-1}$].
  \item $b_V$ : power-law exponent controlling sensitivity of conductance to $h_V$.
  \item $\delta_V$ : additive offset locating the effective stage datum of the conductance modulation; a negative $\delta_V$ implies a threshold stage $h_V = -\delta_V$ below which the modulation factor approaches zero.
...
The offset-power model was fit at the pooled level, estimating $C_V$, $\delta_V$, and $b_V$ by minimizing the sum of squared residuals, with $\delta_V$ constrained so that $h_V + \delta_V > 0$. Per-event offset-power fits are not reported, because the three free parameters are weakly identified within individual closures; the per-event panels of Figure~\ref{fig:compare_fits} (A--C) therefore show only the per-event linear fit, while the pooled offset-power fit is shown alongside the pooled linear fit in panel D.
...
\end{equation}
where $n$ is the number of observations, $\mathrm{SSE}$ is the sum of squared residuals, and $k$ is the number of model parameters ($k = 1$ for the linear model, $k = 3$ for the offset-power model). Lower values indicate a more parsimonious model. We report the pooled AIC and BIC and assess goodness of fit with the centered coefficient of determination, $R^2$.


\section{Results}

Table~\ref{tab:closures} lists the linear conductance $C_L$ (with bootstrapped 95\% confidence intervals) and goodness of fit $R^2_L$ for each closure with at least five daily data points ($N \ge 5$). Figure~\ref{fig:compare_fits} compares the linear and offset-power fits for the three longest closures (panels A--C) and for all closure days pooled (panel D).
In the pooled fit, both AIC and BIC favor the $h_V$ offset-power model over the linear model ($\Delta\mathrm{AIC} = -18.4$, $\Delta\mathrm{BIC} = -10.4$).
However, the improvement in prediction error is modest: the pooled $R^2$ increases from 0.45 (linear) to 0.48 (offset-power on $h_V$), a bootstrapped mean gain of $\Delta R^2 = +0.030$ (95\% CI $[0.013,\,0.055]$).
...
\begin{figure}
    \centering
    \includegraphics[width = \textwidth]{figures/fig6_summary.png}
...
\hline
2014-09-17 & 37 & 28 & 0.79 & [0.70, 0.88] & 0.37 \\
...
Seepage velocities were estimated from monitoring-well data to be 3--4 times faster in sections lacking the seawall (${\sim}80$--110\,m/day) than in sections where it remains intact (${\sim}25$--40\,m/day).
Sediment cores from appendices of the report indicate high permeabilities for sampled beach material, with coefficients of permeability ranging from $K \approx 0.1$--$0.2$\,cm/s (poorly graded sand) to $K \approx 3$--$4$\,cm/s (poorly graded sand with gravel).
These observations are consistent with heterogeneous, locally coarse horizons and potentially conduit-like pathways within the berm, particularly near the mouth opening, and support the finding that effective berm conductance exceeds values predicted from uniform-medium Darcy calculations.
...
$C$ decreases during periods in which the berm was not washed away and rebuilt, notably from 2012 to 2014 and again from 2019 to 2021 (see Figure \ref{fig:summary}).  This rebuilding of the berm can be seen in Figure \ref{fig:berm_crest}, which shows the berm crest elevation as a function of time, using data from four transects across the berm (data from Sonoma County Water Agency).  The lowest values of $C \approx0.5$ m$^2$/s occur in 2014 and 2021, before the berm was partially eroded and rebuilt in 2015 and 2022.
To illustrate how changes in the berm shape between years  contribute to variability in $C$,  Figure \ref{fig:berm_crest}B shows images of closure events in 2012 and 2020.  High $C$ in 2012 coincide with a narrow berm, and low $C$ in 2020 coincide with a wide berm.
 ...
 \begin{figure}
    \centering
    \includegraphics[width = \textwidth]{figures/fig7_interannual_variability.png}
...
We treat the estuary hypsometry as a stage--volume relationship $V(h)$, the estuary water volume at stage $h$, where $h$ is water level (m) and $V(h)$ is volume (m$^3$). From this hypsometric curve we compute the derivative $dV/dh(h)$, which has units of m$^3$/m $=$ m$^2$ and is interpreted as the water-surface area at stage $h$,
\[
A(h) \equiv \frac{dV}{dh}(h),
\]
so that a larger surface area implies a larger change in volume for a given change in stage.

\paragraph{Propagating stage error into volume error.}
Suppose water level has an uncertainty $\delta h$. For sufficiently small errors, a first-order (linear) approximation gives
\[
\delta V \approx \frac{dV}{dh}(h)\,\delta h = A(h)\,\delta h,
\]
where $\delta V$ is the absolute error in volume associated with a stage error $\delta h$, with units of m$^2 \cdot$ m $=$ m$^3$. In the specific example with a 10 cm stage error, $\delta h = 0.10$ m, so at each stage $h$ we estimate
\[
|\delta V(h)| \approx |A(h)|\,\delta h = |A(h)| \times 0.10~\text{m}.
\]

\paragraph{Percent volume error.}
To express this as a percentage error in volume, we compare the absolute error $|\delta V(h)|$ to the actual volume $V(h)$ at that stage:
\[
\text{percent error}(h) = 100 \times \frac{|\delta V(h)|}{V(h)} \approx 100 \times \frac{|A(h)|\,\delta h}{V(h)}.
\]
For very small volumes (near an empty estuary), the ratio $|\delta V|/V(h)$ becomes ill-defined or dominated by numerical noise, so in practice we compute the percentage error only where $V(h) > \varepsilon$ for a small threshold $\varepsilon$ (e.g., $\varepsilon = 10^{-6}$ m$^3$). On those levels, the quantity
\[
\text{pct\_err}_{0.1\text{m}}(h) \approx 100 \times \frac{|A(h)| \times 0.10}{V(h)}
\]
gives the percent error in volume associated with a $\pm 0.10$ m uncertainty in water level at stage $h$.
