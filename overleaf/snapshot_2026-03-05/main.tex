...
We then define an integrated berm hydraulic conductance $\tilde K$ that reflects the combined influences of berm dimensions and material, and examine how $\mathcal{C}$ varies seasonally and interannually.
...
\end{equation}
 ...
 \noindent where $\mathcal{C} =  \frac{K A'_b}{ L'_b}$ ,
...
\[
S \;=\; C_V\left(\frac{h_V+\mathrm{offset}_V}{m_V}\right)^{b_V}\Delta h,
\qquad
m_V=\mathrm{median}(h_V)+\mathrm{offset}_V.
\]
...
\begin{itemize}
  ...
  \item $C_V$ : conductance parameter for the offset--power model.
  \item $b_V$ : power-law exponent controlling sensitivity of conductance to $h_V$.
  \item $\mathrm{offset}_V$ : additive offset representing an effective threshold for flow (shifts $h_V$).
  \item $m_V$ : normalization constant defined from $h_V$ as above.
\end{itemize}
...
To evaluate whether seepage was better predicted by a linear or nonlinear relationship with water level, we compared two models: a linear model of the form $S = \tilde{K} \cdot \Delta h$, and a nonlinear model $S = \tilde{K} \cdot \Delta h \cdot h^b$, where $\Delta h$ is the head difference between estuary and ocean, $h$ is estuary water depth, and $b$ is an exponent varied between 0 and 3 in 0.1 increments. For each closure event, we fit both models and computed the residual sum of squares.

Model performance was evaluated using the Akaike Information Criterion (AIC), which balances model fit and complexity. AIC was calculated as
\begin{equation}
    {AIC} = n \cdot \log({RSS}/n) + 2k,
\end{equation}

where $n$ is the number of observations, ${RSS}$ is the residual sum of squares, and $k$ is the number of model parameters (1 for the linear model, 2 for the exponent model). A lower AIC indicates a more parsimonious model. We report the AIC for both models, as well as the difference ($\Delta {AIC} = {AIC}_{{linear}} - {AIC}_{{exponent}}$) to assess whether inclusion of the exponent improved model performance sufficiently to justify its added complexity.
...
To estimate $\tilde K$ for each closure event,
 we plotted $S$ (inferred through mass balance at daily time intervals) against $\Delta h^b$, and fit a regression line to obtain $\tilde K$ (see Figure \ref{fig:overwash}C for illustration). 
We adjusted the exponent $b$ from 0 to 2, and defined the  best-fit exponent, $b_{optim}$, as the exponent value that maximizes the model coefficient of determination, $R^2$. In the  case with $b = 0$, $\tilde K$ is independent of  $\Delta h$ and can be interpreted as an integrated hydraulic conductivity with dimensions L$^2$/T. 
  

 To estimate $\mathcal{C}$ for each closure event, we compared three models.
...
We optimized the model fit on (i) pooled events, and (ii) separately for each grouped event – median of grouped events is comparable to the pooled events (insert table).

 
In the two shifted power models, $z$ are a dimensionless shape parameters, and $C$ can be interpreted as transmissivity-like. 


 If the data are not filtered to exclude days with wave-driven overwash, $C$ represents an integrated hydraulic conductivity representing both seepage and overwash, $S + O$.  
...
Supporting information Figure S3 compares the results obtained with and without overwash days included in the analysis (i.e., $S$ versus $S + O$). $\tilde K$ values are smaller with overwash included, because the net water loss $O + S$ can underestimate the actual seepage $S$ when overwash offsets part of the volume change.
