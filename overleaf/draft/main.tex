...
\end{equation}
 ...
 \noindent where $C =  \frac{K A'_b}{ L'_b}$ ,
...
\[
S \;=\; C_V\,\zeta_{V}^{\,b_V}\,\Delta h,
\qquad
\zeta_V \equiv \frac{h_V + \delta_V}{m_V},\qquad
m_V = \mathrm{median}(h_V) + \delta_V.
\]
...
\begin{itemize}
  ...
  \item $C_V$ : reference conductance for the offset--power model [$\mathrm{L}^2\,\mathrm{T}^{-1}$].
  \item $b_V$ : power-law exponent controlling sensitivity of conductance to $h_V$.
  \item $\delta_V$ : vertical offset representing the seepage-path vertical dimension when $h_V \to 0$.
  \item $\zeta_V$ : dimensionless scaled water level, $\zeta_V = (h_V + \delta_V)/m_V$.
  \item $m_V$ : median normalization, $m_V = \mathrm{median}(h_V) + \delta_V$.
\end{itemize}
...
We estimated parameters for both models using two strategies: (i) a \textbf{pooled fit}, in which one parameter set is estimated from all closure events simultaneously, and (ii) \textbf{per-event fits}, in which parameters are estimated separately within each closure event and then summarized across events.
...
For the offset--power model, $C_V$, $\delta_V$, and $b_V$ were estimated by minimizing the sum of squared residuals, with $\delta_V$ bounded to keep the base $h_V + \delta_V > 0$ and capped at the site-scale data spread.

Model performance was evaluated using the Akaike Information Criterion (AIC) and Bayesian Information Criterion (BIC), computed as
\begin{equation}
    \mathrm{AIC} = n \cdot \ln(\mathrm{SSE}/n) + 2k,
    \qquad
    \mathrm{BIC} = n \cdot \ln(\mathrm{SSE}/n) + k \ln n,
\end{equation}
where $n$ is the number of observations, $\mathrm{SSE}$ is the sum of squared residuals, and $k$ is the number of model parameters ($k = 1$ for the linear model, $k = 3$ for the offset--power model). Lower values indicate a more parsimonious model. For per-event fits, event-level AIC and BIC were summed. Goodness of fit was assessed using centered $R^2$ and RMSE, with both unweighted and duration-weighted summaries across events.

 If the data are not filtered to exclude days with wave-driven overwash, $C$ represents an integrated hydraulic conductance representing both seepage and overwash, $S + O$.  
 


\section{Results}

Table~\ref{tab:closures} lists per-event linear-model results ($C_L$ and $R^2$) and the best-fit offset--power exponent $b_{optim}$ with its corresponding $R^2_{optim}$ for all closures with at least 5 days of data.
Both AIC and BIC favor the $h_V$ offset--power model over the linear model in pooled fits ($\Delta\mathrm{AIC} \approx -19$ to $-26$, $\Delta\mathrm{BIC} \approx -11$ to $-18$).
However, the improvement in prediction error is modest: the pooled $R^2$ increases from 0.43 (linear) to 0.45 (offset--power on $h_V$).
% This is illustrated by Figure \ref{fig:compare_fits}, which compares model fits for the three longest closures (panels A--C) and all closure events combined (panel D).
Given the comparable errors, the linear model $S = C_L\,\Delta h$ provides a parsimonious baseline, while the offset--power model captures additional structure in the conductance--water-level relationship.
...
\begin{figure}
    ...
    (B) Integrated hydraulic conductance $C_L$ determined through regression of $S$ on $\Delta h$ for each closure event.  
...
Supporting information Figure S3 compares the results obtained with and without overwash days included in the analysis (i.e., $S$ versus $S + O$). $C_L$ values are smaller with overwash included, because the net water loss $O + S$ can underestimate the actual seepage $S$ when overwash offsets part of the volume change.
...
\begin{table}[]    
    \centering
\caption{Summary statistics for the 30 longest closures, including closure start date, duration and number of data points $N$. $C_L$ and $R^2$ columns indicate integrated hydraulic conductance and $R^2$ value for the linear model.  $b_{optim}$ is the exponent that achieves maximum $R^2$ value, $R^2_{optim}$. }   
    \small
\begin{tabular}{lllll|ll}
\hline
Date & Duration & $N$ & $C_L$ & $R^2$ & $b_{optim}$ & $R^2_{optim}$ \\
\hline
...
The strong performance of the linear model ($S = C_L\,\Delta h$) indicates that the effective conductance does not change dramatically over the observed range of estuary water levels; that is, increasing estuary water level does not significantly alter the ratio of cross-sectional flow area to flow length, $A_b/L_b$. This could occur due to a weak dependence of $A_b$ and $L_b$ on water level, or because any dependencies of $A_b$ and $L_b$ on water level approximately cancel each other out. 
...
Such conduits could be remnants of a failed rock-wall jetty \citep{behrens2009characterization} or naturally occurring cobble/gravel horizons beneath the berm.  It is unlikely that the berm is homogeneous, and more realistically, the value of $C$ represents an integration of non-uniform hydraulic conductance over the flow region. 

 $C$ varies on both seasonal and interannual timescales, reflecting changes in berm dimensions and hydraulic conductance.
 % In other years, the sand barrier was partially eroded and rebuilt. 
$C$ decreases during periods in which the berm was not washed away and rebuilt, notably from 2012 to 2014 and again from 2019 to 2021 (see Figure \ref{fig:summary}).  This rebuilding of the berm can be seen in in Figure \ref{fig:berm_crest}, which shows the berm crest elevation as a function of time, using data from four transects across the berm (data from Sonoma County Water Agency).  The lowest values of $C \approx0.5$ m$^2$/s occur in 2014 and 2021, before the berm was partially eroded and rebuilt in 2015 and 2022.
To illustrate how changes in the berm shape between years  contribute to variability in $C$,  Figure \ref{fig:berm_crest}B shows images of closure events in 2012 and 2020.  High $C$ in 2012 coincide with a narrow berm, and low $C$ in 2020 coincide with a wide berm. 
 
 $C$ decreases over the dry season in 2016 and 2017. 
 The highest values at the start of these years followed a wet winter in which the berm was partially eroded as the mouth migrated north from the jetty, followed by rebuilding of a large portion of the berm north of the jetty. Compared with interannual changes, seasonal changes in the berm shape are relatively small  (see Figure \ref{fig:2018_closures}, showing photos of two closure events separated by approximately one month), consistent with smaller changes in  $C$  over a season than between years. Given that change in the berm dimensions is limited, larger changes in  $C$  are likely due to clogging of the sand barrier. However, if indeed the primary seepage flux is via a preferential cobble/gravel/fractured rock flow pathway, then clogging must involve sand as well as finer particles. Further research is needed to better characterize this seasonal clogging, which can be accumulated over subsequent years. Finer sediment may be transported into the berm, or reduced hydraulic conductance may result from deposition of a superficial layer of sand, e.g., an apron of sand created by wave overwash would both thicken the berm and produce a low-conductance veneer over pre-existing high-conductance `pipes'. 
%These controls on seepage require further attention. 
 % It is notable that persistent closures are more likely in dry years with low summer river flow, which are also likely years of low $C$ necessitating higher water levels in the estuary to drive seepage loss.
   % The highest values of $C \approx$1.5 m$^2$/s occur after major winter flow events (e.g., early 2017, in which peak flow exceeded 1500 m$^3$/s and washed away the berm).

 \begin{figure}
    \centering
    \includegraphics[width = \textwidth]{figures/interannual_variability.png}
    \caption{Berm crest elevation as a function of time, showing the median (blue dots) and range of  four transects across the berm. Photos taken on November 11, 2012 and December 5, 2020, illustrating larger interannual changes in the berm shape.  In 2012, large $C$ is consistent with a narrowing berm by the jetty.  Likewise, in 2020, small $C$ is consistent with the wider berm shape.   }
...
%In years with higher river flow, a steady state could potentially  be achieved if water level rose to about 3 m above ocean water level (assuming $Q_r = 3$ m$^3$/s and $C_L$ = 1 m$^2$/s, $\Delta h = S / C_L = 3 $ m). 
...
We then estimated an integrated hydraulic conductance $C$ by regression of the seepage rate $S$ on the head difference between ocean and estuary, $\Delta h$. 
...
We suggest that interannual variability in $C$ reflects variations in berm shapes, and seasonal variability reflects changes in the composition and hydraulic conductance of the berm (e.g., clogging by fine particles).
