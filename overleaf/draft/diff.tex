...
%DIF DEL snapshot_2026-03-05/main.tex   Thu Mar  5 22:27:13 2026
%DIF ADD draft/main.tex                 Fri Mar  6 06:44:59 2026
...
Here, we estimate seepage rates through the berm of the Russian River estuary, a wave-dominated system in northern California that closes 1–15 times annually for 1–30 days. Using a mass-balance approach and Darcy’s Law, we define an integrated hydraulic conductance, \DIFdelbegin \DIFdel{$\mathcal{C}$}\DIFdelend \DIFaddbegin \DIFadd{$C$}\DIFaddend , that captures the combined influence of the berm geometry and material properties. Across 35 closure events, seepage accounted for 20–60\% of river inflow, with average daily rates ranging from 0.5 to 3.0 m$^3$/s and $C$ values between 0.5 and 1.5 m$^2$/s. In some years, $C$ declined progressively over successive closures, suggesting seasonal clogging by fine sediments. Interannual variation in $C$ was associated with changes in berm shape following winter erosion and rebuilding. These results support the use of a linear head–seepage relationship in the Russian River Estuary, and highlight how geomorphological and sedimentary changes jointly influence estuarine hydrodynamics.

% Here, we use a mass-balance approach to estimate seepage through the sand barrier (berm) of the Russian River estuary, a  wave-dominated estuary in northern California that closes 1-15 times per year, with closure durations of 1-30 days.  Modeling seepage through the berm using Darcy's Law, we estimate an `integrated' berm hydraulic conductance $C$ that reflects the combined influences of berm dimensions and sand conductivity. Seasonally, $C$ decreases over subsequent closures, possibly due to clogging of the sand barrier by fine sediments. This affect can persist over multiple years, although interannual variation in $C$ likely includes variation in the shape of the berm, which can be washed away during winters with heavy rains and reform the following spring.
...
\item The integrated hydraulic \DIFdelbegin \DIFdel{conductivity }\DIFdelend \DIFaddbegin \DIFadd{conductance }\DIFaddend of the berm decreases seasonally over subsequent inlet closures
...
% Seepage through the sand barrier can also contriubte to water loss from a closed estuary.
...
River discharge is managed for water supply in summer and to mitigate flood risk in winter (CITE). 
...
These tradeoffs between flood risk and marsh accretion highlight the need to better understand water loss mechanisms during closure periods. Here, we quantify water loss from the Russian River during 35 persistent closure events, using 12 years of compiled data on river flow rates and estuary and ocean water levels. 
...
% consistent with head-driven seepage governed by Darcy’s Law.
We then define an integrated berm hydraulic conductance \DIFdelbegin \DIFdel{$\tilde K$ }\DIFdelend \DIFaddbegin \DIFadd{$C$ }\DIFaddend that reflects the combined influences of berm dimensions and material, and examine how \DIFdelbegin \DIFdel{$\mathcal{C}$ }\DIFdelend \DIFaddbegin \DIFadd{$C$ }\DIFaddend varies seasonally and interannually.
...
\begin{figure}
    \centering
    \includegraphics[width = \textwidth]{figures/closure_statistics.png}
    \caption{
...
We computed $V$ as a function of the estuary water level $h$ [L] using hypsometric data \citep{EDS_2011_LowerRussianRiver_Bathy}.
...
A daily timestep was used as travel times through berm are expected to be of the order of a day (estimated as $L_b/K$, assuming sand $K \approx$ 45 m/day and berm width $\approx 50$ m), thus smoothing out the effect of tidal, diurnal and other high-frequency fluctuations on seepage rate. 


To identify upper limits of daily evaporation from the estuary, we estimated the estuary area as 1 km$^2$ and the maximum  evaporation rate as 10 mm/day, which yields $E A_e \approx 0.1$ m$^3$/s. Accordingly, we did not include evaporation in the analysis, because this term is likely an order of magnitude smaller than typical river discharge during closures ($Q_r \approx 1-5$ m$^3$/s).
...
As the estuary water level rises, the hydraulic gradient increases, and one may also expect $A_b$ or $L_b$ to vary, yielding a nonlinear dependence of $S$ on estuary water level  $ h$. To make any such dependencies explicit, $A_b$ and $L_b$ are expressed as $A_b = A'_b  h^x$ and $L_b = L'_b  h^y$, where $A'_b$ and $L'_b$ are constants and the exponents $x$ and $y$ account for any potential dependencies of $A_b$ and $W_b$ on $ h$. 
Equation \ref{eq:Darcy} is rewritten:

\begin{equation}
    S = K (A'_b  h^x) \frac{\Delta h}{ (L'_b  h^y)}.
    \label{eq:Darcy_w_exponents}
\end{equation}

However, the flow cross-sectional area and berm width are unknown, as are their dependencies on estuary water level. The cross sectional area is anticipated to increase with increasing estuary water level ($x >0$) and the berm width is expected to decrease ($y < 0$) .  While the berm shape can be estimated from aerial or satellite imagery, this likely differs from its hydraulically-active area. We therefore estimate an effective hydraulic conductance \DIFdelbegin \DIFdel{$\mathcal{C}$}\DIFdelend \DIFaddbegin \DIFadd{$C$}\DIFaddend :

\begin{equation}
    S = \DIFdelbegin \DIFdel{\mathcal{C} }\DIFdelend \DIFaddbegin \DIFadd{C\,}\DIFaddend \Delta h,
    \label{eq:tildeK}
\end{equation}

 \noindent where \DIFdelbegin \DIFdel{$\mathcal{C} =  \frac{K A'_b}{ L'_b}$ }\DIFdelend \DIFaddbegin \DIFadd{$C =  \frac{K A'_b}{ L'_b}$ }\DIFaddend ,

 Because mouth--scale geometry is unavailable and the analysis is restricted to closed--mouth periods, we do not model berm dimensions explicitly. Instead, we test whether estuary water level $h_V$ serves as a proxy for the {seepage cross--section} (and thus for the conductance $C$), by comparing a constant--conductance model to a shifted--power form $C(h_V)$.


 % Two candidate seepage models (with variable definitions)

\paragraph{Linear (constant conductance).}
\[
S \;=\; C_L\,\Delta h.
\]

\paragraph{Offset-power on visitor-center water level $h_V$.}
\[
S \;=\; C_V\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdel{\frac{h_V+\mathrm{offset}_V}{m_V}}%DIFDELCMD < \right)%%%
\DIFdel{^{b_V}}\DIFdelend \DIFaddbegin \DIFadd{\,\zeta_{V}^{\,b_V}\,}\DIFaddend \Delta h,
\qquad
\DIFaddbegin \DIFadd{\zeta_V \equiv \frac{h_V + \delta_V}{m_V},\qquad
}\DIFaddend m_V = \mathrm{median}(h_V) + \DIFdelbegin \DIFdel{\mathrm{offset}}\DIFdelend \DIFaddbegin \DIFadd{\delta}\DIFaddend _V.
\]

\paragraph{Definitions.}
\begin{itemize}
  \item $S$ : seepage discharge (outflow from the estuary through the barrier).
  \item $\Delta h$ : water-level difference driving flow (estuary stage minus ocean stage).
  \item $C_L$ : constant (linear) conductance parameter.
  \item $h_V$ : visitor-center water level (estuary stage metric).
  \item $C_V$ : \DIFdelbegin \DIFdel{conductance parameter }\DIFdelend \DIFaddbegin \DIFadd{reference conductance }\DIFaddend for the offset--power model \DIFaddbegin [\DIFadd{$\mathrm{L}^2\,\mathrm{T}^{-1}$}]\DIFaddend .
  \item $b_V$ : power-law exponent controlling sensitivity of conductance to $h_V$.
  \item \DIFdelbegin \DIFdel{$\mathrm{offset}_V$ : additive offset representing an effective threshold for flow (shifts $h_V$)}\DIFdelend \DIFaddbegin \DIFadd{$\delta_V$ : vertical offset representing the seepage-path vertical dimension when $h_V \to 0$}\DIFaddend .
  \item \DIFaddbegin \DIFadd{$\zeta_V$ : dimensionless scaled water level, $\zeta_V = (h_V + \delta_V)/m_V$.
  }\item \DIFaddend $m_V$ : \DIFdelbegin \DIFdel{normalization constant defined from $h_V$ as above}\DIFdelend \DIFaddbegin \DIFadd{median normalization, $m_V = \mathrm{median}(h_V) + \delta_V$}\DIFaddend .
\end{itemize}


 % The exponent $b$ accounts for any dependencies of $A_b$ and $L_b$ on $h$ (i.e., the exponents $x$ and $y$, with $b =  x - y$). 
 % Expect L_b decreases with increasing water level. Also expect area to increase with increasing h, so expect b is positive.


\DIFdelbegin \DIFdel{To evaluate whether seepage was better predicted by a linear or nonlinear relationship with water level, we compared two models : a linear model of the form $S = \tilde{K} \cdot \Delta h$, and a nonlinear model $S = \tilde{K} \cdot \Delta h \cdot h^b$, where $\Delta h$ is the head difference between estuary and ocean, $h$ is estuary water depth, and $b$ is an exponent varied between 0 }\DIFdelend \DIFaddbegin \DIFadd{We estimated parameters for both models using two strategies: (i) a }\textbf{\DIFadd{pooled fit}}\DIFadd{, in which one parameter set is estimated from all closure events simultaneously, }\DIFaddend and \DIFdelbegin \DIFdel{3 in 0.1 increments. For }\DIFdelend \DIFaddbegin \DIFadd{(ii) }\textbf{\DIFadd{per-event fits}}\DIFadd{, in which parameters are estimated separately within }\DIFaddend each closure event \DIFaddbegin \DIFadd{and then summarized across events.
In the linear case, the conductance $C_L$ was estimated by through-origin ordinary least squares regression of $S$ on $\Delta h$ (see Figure~\ref{fig:overwash}C for illustration).
For the offset--power model, $C_V$}\DIFaddend , \DIFdelbegin \DIFdel{we fit both models and computed the residual sum of squares}\DIFdelend \DIFaddbegin \DIFadd{$\delta_V$, and $b_V$ were estimated by minimizing the sum of squared residuals, with $\delta_V$ bounded to keep the base $h_V + \delta_V > 0$ and capped at the site-scale data spread}\DIFaddend .

Model performance was evaluated using the Akaike Information Criterion (AIC) \DIFdelbegin \DIFdel{, which balances model fit and complexity. AIC was calculated as
}\DIFdelend \DIFaddbegin \DIFadd{and Bayesian Information Criterion (BIC), computed as
}\DIFaddend \begin{equation}
    \DIFdelbegin %DIFDELCMD < {%%%
\DIFdel{AIC}%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{\mathrm{AIC} }\DIFaddend = n \cdot \DIFdelbegin \DIFdel{\log}\DIFdelend \DIFaddbegin \DIFadd{\ln}\DIFaddend (\DIFdelbegin %DIFDELCMD < {%%%
\DIFdel{RSS}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{\mathrm{SSE}}\DIFaddend /n) + 2k,
    \DIFaddbegin \DIFadd{\qquad
    \mathrm{BIC} = n \cdot \ln(\mathrm{SSE}/n) + k \ln n,
}\DIFaddend \end{equation}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend where $n$ is the number of observations, \DIFdelbegin \DIFdel{${RSS}$ is the residual sum of squares}\DIFdelend \DIFaddbegin \DIFadd{$\mathrm{SSE}$ is the sum of squared residuals}\DIFaddend , and $k$ is the number of model parameters (\DIFdelbegin \DIFdel{1 }\DIFdelend \DIFaddbegin \DIFadd{$k = 1$ }\DIFaddend for the linear model, \DIFdelbegin \DIFdel{2 for the exponent }\DIFdelend \DIFaddbegin \DIFadd{$k = 3$ for the offset--power }\DIFaddend model). \DIFdelbegin \DIFdel{A lower AIC indicates }\DIFdelend \DIFaddbegin \DIFadd{Lower values indicate }\DIFaddend a more parsimonious model. \DIFdelbegin \DIFdel{We report the AIC for both models, as well as the difference ($\Delta {AIC} = {AIC}_{{linear}} - {AIC}_{{exponent}}$) to assess whether inclusion of the exponent improved model performance sufficiently to justify its added complexity.
 A positive $\Delta \text{AIC}$ indicates support for the exponent model. Following conventional thresholds, $\Delta \text{AIC} > 2$ suggests substantial support, while values greater than 10 indicate strong support for the more complex model.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\textbf{\DIFdel{Previous:}}
%DIFAUXCMD
\DIFdel{To estimate $\tilde K$ for each closure event,
 we plotted $S$ (inferred through mass balance at daily time intervals) against $\Delta h^b$, and fit a regression line to obtain $\tilde K$ (see Figure \ref{fig:overwash}C for illustration). We adjusted the exponent $b$ from 0 to 2, and defined the  best-fit exponent, $b_{optim}$, as the exponent value that maximizes the model coefficient of determination, }\DIFdelend \DIFaddbegin \DIFadd{For per-event fits, event-level AIC and BIC were summed. Goodness of fit was assessed using centered }\DIFaddend $R^2$ \DIFdelbegin \DIFdel{. In the  case with $b = 0$, $\tilde K$ is independent of  $\Delta h$ and can be interpreted as an integrated hydraulic conductivity with dimensions L$^2$/T. 
  }%DIFDELCMD < 

%DIFDELCMD <  %%%
\DIFdel{To estimate $\mathcal{C}$ for each closure event, we compared three models.
 In the linear case, we plotted $S$ (inferred through mass balance at daily time intervals) against $\Delta h$, and fit a regression line to obtain $C_{L}$ (see Figure \ref{fig:overwash}C for illustration). 
We optimized the model fit on (i) pooled events, and (ii) separately for each grouped event – median of grouped events is comparable to the pooled events (insert table)}\DIFdelend \DIFaddbegin \DIFadd{and RMSE, with both unweighted and duration-weighted summaries across events}\DIFaddend .

 \DIFdelbegin \DIFdel{In the two shifted power models, $z$ are a dimensionless shape parameters, and $C$ can be interpreted as transmissivity-like. 
}%DIFDELCMD < 

%DIFDELCMD <  %%%
\DIFdelend If the data are not filtered to exclude days with wave-driven overwash, $C$ represents an integrated hydraulic \DIFdelbegin \DIFdel{conductivity }\DIFdelend \DIFaddbegin \DIFadd{conductance }\DIFaddend representing both seepage and overwash, $S + O$.  



\section{Results}

\DIFdelbegin \DIFdel{The best-fit exponent $b$ differs between closures (see Table\ref{tab:closures}, which lists $\tilde K$ }\DIFdelend \DIFaddbegin \DIFadd{Table~\ref{tab:closures} lists per-event linear-model results ($C_L$ }\DIFaddend and $R^2$\DIFdelbegin \DIFdel{for the linear case, as well as }\DIFdelend \DIFaddbegin \DIFadd{) and the best-fit offset--power exponent }\DIFaddend $b_{optim}$ \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{with its corresponding }\DIFaddend $R^2_{optim}$ for \DIFdelbegin \DIFdel{the best-fit case, for }\DIFdelend all closures with at least 5 days of data\DIFaddbegin \DIFadd{.
Both AIC and BIC favor the $h_V$ offset--power model over the linear model in pooled fits ($\Delta\mathrm{AIC} \approx -19$ to $-26$, $\Delta\mathrm{BIC} \approx -11$ to $-18$}\DIFaddend ).
\DIFdelbegin \DIFdel{The average of the best-fit values is $b = 1.25$; however, the difference in prediction errors is minor, with the ensemble mean $R^2=0.43$ for $b = 1$ and  $R^2=0.45$ for $b = 1.25$.
This is illustrated by Figure \ref{fig:compare_fits}, which compares different values of the exponent $b$ for the three longest closures (panels A-C) and all of the closure events combined (panel D).
}\DIFdelend \DIFaddbegin \DIFadd{However, the improvement in prediction error is modest: the pooled $R^2$ increases from 0.43 (linear) to 0.45 (offset--power on $h_V$).
%DIF >  This is illustrated by Figure \ref{fig:compare_fits}, which compares model fits for the three longest closures (panels A--C) and all closure events combined (panel D).
}\DIFaddend Given the comparable errors\DIFdelbegin \DIFdel{for $b = 1$ and 1.25, we adopt a linear fit, which  has the advantage that $\tilde K$ is interpretable as a hydraulic conductivity with dimensions L$^2$/T}\DIFdelend \DIFaddbegin \DIFadd{, the linear model $S = C_L\,\Delta h$ provides a parsimonious baseline, while the offset--power model captures additional structure in the conductance--water-level relationship}\DIFaddend .
...
\begin{figure}
    ...
    (B) Integrated hydraulic \DIFdelbeginFL \DIFdelFL{conductivity }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{conductance $C_L$ }\DIFaddendFL determined through regression of $S$ on $\Delta h$ for each closure event.  
...
Figure \ref{fig:summary} shows how the mean seepage rate $S$  (panel A) and the integrated hydraulic \DIFdelbegin \DIFdel{conductivity $\tilde K$ }\DIFdelend \DIFaddbegin \DIFadd{conductance $C_L$ }\DIFaddend (panel B) vary over time for all closures with  at least 5 days of data. Seepage rates vary from 0.5 to 3.0 m$^3$/s, with the highest rates in 2017. \DIFdelbegin \DIFdel{$\tilde K$ }\DIFdelend \DIFaddbegin \DIFadd{$C_L$ }\DIFaddend remains relatively consistent across closures, ranging from 0.5 to 1.5 m$^2$/s. The highest \DIFdelbegin \DIFdel{conductivity }\DIFdelend \DIFaddbegin \DIFadd{conductance }\DIFaddend was observed in 2017, consistent with the  high seepage rates that year. In some years, \DIFdelbegin \DIFdel{$\tilde K$ }\DIFdelend \DIFaddbegin \DIFadd{$C_L$ }\DIFaddend decreases over subsequent closures (notably, in 2016 and 2017), reflecting a seasonal change in $K$, $A_b$, and/or \DIFdelbegin \DIFdel{$W_b$}\DIFdelend \DIFaddbegin \DIFadd{$L_b$}\DIFaddend . 
...
Supporting information Figure S3 compares the results obtained with and without overwash days included in the analysis (i.e., $S$ versus $S + O$). \DIFdelbegin \DIFdel{$\tilde K$ }\DIFdelend \DIFaddbegin \DIFadd{$C_L$ }\DIFaddend values are smaller with overwash included, because the net water loss $O + S$ can underestimate the actual seepage $S$ when overwash offsets part of the volume change.
% $O+S < S$, and not representative of actual seepage flows.
Nevertheless, the results are qualitatively similar, showing the same seasonal and interannual trends in \DIFdelbegin \DIFdel{$\tilde K$}\DIFdelend \DIFaddbegin \DIFadd{$C_L$}\DIFaddend .
 % Orange markers show the data filtered to exclude overwash ($S$), and blue shows the unfiltered results ($S + O$).  

\begin{table}[]    
    \centering
\caption{Summary statistics for the 30 longest closures, including closure start date, duration and number of data points $N$. \DIFdelbeginFL \DIFdelFL{$\tilde K$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$C_L$ }\DIFaddendFL and $R^2$ columns indicate integrated hydraulic \DIFdelbeginFL \DIFdelFL{conductivity }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{conductance }\DIFaddendFL and $R^2$ value \DIFdelbeginFL \DIFdelFL{determined fitting Equation 6 with $b = 1$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for the linear model}\DIFaddendFL .  $b_{optim}$ is \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL exponent that achieves maximum $R^2$ value, $R^2_{optim}$. }   
    \small
\begin{tabular}{lllll|ll}
\hline
Date & Duration & $N$ & \DIFdelbeginFL \DIFdelFL{$\tilde K$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$C_L$ }\DIFaddendFL & $R^2$ & $b_{optim}$ & $R^2_{optim}$ \\
\hline
...
The \DIFdelbegin \DIFdel{result that $b \approx 1$ in Equation \ref{eq:tildeK} indicates that }\DIFdelend \DIFaddbegin \DIFadd{strong performance of the linear model ($S = C_L\,\Delta h$) indicates that the effective conductance does not change dramatically over the observed range of estuary water levels; that is, }\DIFaddend increasing estuary water level does not significantly alter the ratio of cross-sectional flow area to flow length, \DIFdelbegin \DIFdel{$A_b$/$W_b$}\DIFdelend \DIFaddbegin \DIFadd{$A_b/L_b$}\DIFaddend . This could occur due to a weak dependence of $A_b$ and \DIFdelbegin \DIFdel{$W_b$ on $\Delta h$}\DIFdelend \DIFaddbegin \DIFadd{$L_b$ on water level}\DIFaddend , or because any dependencies of $A_b$ and $L_b$ on \DIFdelbegin \DIFdel{$\Delta h$ }\DIFdelend \DIFaddbegin \DIFadd{water level approximately }\DIFaddend cancel each other out. 
...
Such conduits could be remnants of a failed rock-wall jetty \citep{behrens2009characterization} or naturally occurring cobble/gravel horizons beneath the berm.  It is unlikely that the berm is homogeneous, and more realistically, the value of \DIFdelbegin \DIFdel{$\tilde K$ }\DIFdelend \DIFaddbegin \DIFadd{$C$ }\DIFaddend represents an integration of non-uniform hydraulic \DIFdelbegin \DIFdel{conductivity }\DIFdelend \DIFaddbegin \DIFadd{conductance }\DIFaddend over the flow region. 

 \DIFdelbegin \DIFdel{$\tilde{K}$ }\DIFdelend \DIFaddbegin \DIFadd{$C$ }\DIFaddend varies on both seasonal and interannual timescales, reflecting changes in berm dimensions and hydraulic \DIFdelbegin \DIFdel{conductivity}\DIFdelend \DIFaddbegin \DIFadd{conductance}\DIFaddend .
 % In other years, the sand barrier was partially eroded and rebuilt. 
\DIFdelbegin \DIFdel{$\tilde K$ }\DIFdelend \DIFaddbegin \DIFadd{$C$ }\DIFaddend decreases during periods in which the berm was not washed away and rebuilt, notably from 2012 to 2014 and again from 2019 to 2021 (see Figure \ref{fig:summary}).  This rebuilding of the berm can be seen in in Figure \ref{fig:berm_crest}, which shows the berm crest elevation as a function of time, using data from four transects across the berm (data from Sonoma County Water Agency).  The lowest values of \DIFdelbegin \DIFdel{$\tilde K \approx0.5$ }\DIFdelend \DIFaddbegin \DIFadd{$C \approx0.5$ }\DIFaddend m$^2$/s occur in 2014 and 2021, before the berm was partially eroded and rebuilt in 2015 and 2022.
To illustrate how changes in the berm shape between years  contribute to variability in \DIFdelbegin \DIFdel{$\tilde K$}\DIFdelend \DIFaddbegin \DIFadd{$C$}\DIFaddend ,  Figure \ref{fig:berm_crest}B shows images of closure events in 2012 and 2020.  High \DIFdelbegin \DIFdel{$\tilde K$ }\DIFdelend \DIFaddbegin \DIFadd{$C$ }\DIFaddend in 2012 coincide with a narrow berm, and low \DIFdelbegin \DIFdel{$\tilde K$ }\DIFdelend \DIFaddbegin \DIFadd{$C$ }\DIFaddend in 2020 coincide with a wide berm. 

 \DIFdelbegin \DIFdel{$\tilde K$ }\DIFdelend \DIFaddbegin \DIFadd{$C$ }\DIFaddend decreases over the dry season in 2016 and 2017. 
 The highest values at the start of these years followed a wet winter in which the berm was partially eroded as the mouth migrated north from the jetty, followed by rebuilding of a large portion of the berm north of the jetty. Compared with interannual changes, seasonal changes in the berm shape are relatively small  (see Figure \ref{fig:2018_closures}, showing photos of two closure events separated by approximately one month), consistent with smaller changes in  \DIFdelbegin \DIFdel{$\tilde K$  }\DIFdelend \DIFaddbegin \DIFadd{$C$  }\DIFaddend over a season than between years. Given that change in the berm dimensions is limited, larger changes in  \DIFdelbegin \DIFdel{$\tilde K$  }\DIFdelend \DIFaddbegin \DIFadd{$C$  }\DIFaddend are likely due to clogging of the sand barrier. However, if indeed the primary seepage flux is via a preferential cobble/gravel/fractured rock flow pathway, then clogging must involve sand as well as finer particles. Further research is needed to better characterize this seasonal clogging, which can be accumulated over subsequent years. Finer sediment may be transported into the berm, or reduced hydraulic \DIFdelbegin \DIFdel{conductivity }\DIFdelend \DIFaddbegin \DIFadd{conductance }\DIFaddend may result from deposition of a superficial layer of sand, e.g., an apron of sand created by wave overwash would both thicken the berm and produce a \DIFdelbegin \DIFdel{low-conductivity }\DIFdelend \DIFaddbegin \DIFadd{low-conductance }\DIFaddend veneer over pre-existing \DIFdelbegin \DIFdel{high-conductivity }\DIFdelend \DIFaddbegin \DIFadd{high-conductance }\DIFaddend `pipes'. 
%These controls on seepage require further attention. 
 %DIF <  It is notable that persistent closures are more likely in dry years with low summer river flow, which are also likely years of low $\tilde K$ necessitating higher water levels in the estuary to drive seepage loss.
   %DIF <  The highest values of $\tilde K \approx$1.5 m$^2$/s occur after major winter flow events (e.g., early 2017, in which peak flow exceeded 1500 m$^3$/s and washed away the berm).
%DIF >  It is notable that persistent closures are more likely in dry years with low summer river flow, which are also likely years of low $C$ necessitating higher water levels in the estuary to drive seepage loss.
   %DIF >  The highest values of $C \approx$1.5 m$^2$/s occur after major winter flow events (e.g., early 2017, in which peak flow exceeded 1500 m$^3$/s and washed away the berm).

 \begin{figure}
    \centering
    \includegraphics[width = \textwidth]{figures/interannual_variability.png}
    \caption{Berm crest elevation as a function of time, showing the median (blue dots) and range of  four transects across the berm. Photos taken on November 11, 2012 and December 5, 2020, illustrating larger interannual changes in the berm shape.  In 2012, large \DIFdelbeginFL \DIFdelFL{$\tilde K$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$C$ }\DIFaddendFL is consistent with a narrowing berm by the jetty.  Likewise, in 2020, small \DIFdelbeginFL \DIFdelFL{$\tilde K$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$C$ }\DIFaddendFL is consistent with the wider berm shape.   }
...
%DIF < In years with higher river flow, a steady state could potentially  be achieved if water level rose to about 3 m above ocean water level (assuming $Q_r = 3$ m$^3$/s and $\tilde K$ = 1 m$^2$/s, $\Delta h = S / \tilde K = 3 $ m). 
%DIF > In years with higher river flow, a steady state could potentially  be achieved if water level rose to about 3 m above ocean water level (assuming $Q_r = 3$ m$^3$/s and $C_L$ = 1 m$^2$/s, $\Delta h = S / C_L = 3 $ m). 
...
% might be worth mentioning in Section 1 that the water balance in lagoons like the Russian could change in the future if evaporative losses increase (due to warmer temps), and if summertime inflows decrease and the annual 'dry' season gets longer (I think this is where the GCM models were heading last I saw, but should confirm). If those are both true, seepage loss rates could contribute to longer-duration closure events, and lower water levels in the lagoon, compared to existing conditions.
 ...
 Seepage losses are likely significant in other ICEs where the water level rises significantly above the ocean water level, which occurs where river inflow exceeds evaporative losses during closure periods.  Different barrier configurations, in terms of dimensions and sediment types, are expected to result in different flow rates.  Likewise, other ICEs may exhibit differences in how \DIFdelbegin \DIFdel{$\tilde K$ }\DIFdelend \DIFaddbegin \DIFadd{$C$ }\DIFaddend changes over seasons and years, due to clogging and/or changes in berm shape. While some data exist for similar water-balance analyses in other California ICEs, new work should also include data collection that can better define and demonstrate controls on the seepage process.
...
We then estimated an integrated hydraulic \DIFdelbegin \DIFdel{conductivity $\tilde K$ }\DIFdelend \DIFaddbegin \DIFadd{conductance $C$ }\DIFaddend by regression of the seepage rate $S$ on the head difference between ocean and estuary, $\Delta h$. 
...
We suggest that interannual variability in \DIFdelbegin \DIFdel{$\tilde K$ }\DIFdelend \DIFaddbegin \DIFadd{$C$ }\DIFaddend reflects variations in berm shapes, and seasonal variability reflects changes in the composition and hydraulic \DIFdelbegin \DIFdel{conductivity }\DIFdelend \DIFaddbegin \DIFadd{conductance }\DIFaddend of the berm (e.g., clogging by fine particles).
...
We treat the estuary hypsometry as a stage–volume relationship
\[
V(h) = \text{estuary water volume at stage } h,
\]
where \(h\) is water level (m) and \(V(h)\) is volume (m\(^3\)).

From this hypsometric curve we compute the derivative
\[
\frac{dV}{dh}(h),
\]
which has units
\[
\frac{\text{m}^3}{\text{m}} = \text{m}^2.
\]
This derivative is interpreted as the **water‐surface area** at stage \(h\),
\[
A(h) \equiv \frac{dV}{dh}(h),
\]
so that larger surface area implies a larger change in volume for a given change in stage.

---

 Propagating stage error into volume error

Suppose that water level has an uncertainty \(\delta h\). For sufficiently small
errors, a first‐order (linear) approximation gives
\[
\delta V \approx \frac{dV}{dh}(h)\,\delta h = A(h)\,\delta h.
\]
Here \(\delta V\) is the absolute error in volume associated with a stage error
\(\delta h\), and has units
\[
[\delta V] = [A(h)]\,[\delta h]
           = \text{m}^2 \cdot \text{m}
           = \text{m}^3.
...
In the specific example with a 10 cm stage error,
\[
\delta h = 0.10~\text{m},
\]
so at each stage \(h\) we estimate
\[
|\delta V(h)| \approx |A(h)| \,\delta h
                  = |A(h)| \times 0.10~\text{m}.
\]
...
To express this as a **percentage** error in volume, we compare the absolute
error \(|\delta V(h)|\) to the actual volume \(V(h)\) at that stage:
\[
\text{percent error}(h)
= 100 \times \frac{|\delta V(h)|}{V(h)}
\approx 100 \times \frac{|A(h)|\,\delta h}{V(h)}.
\]

For very small volumes (near empty estuary), the ratio
\(|\delta V| / V(h)\) becomes ill‐defined or dominated by numerical noise,
so in practice we only compute the percentage error where
\[
V(h) > \varepsilon,
\]
for some small threshold \(\varepsilon\) (e.g. \(\varepsilon = 10^{-6}\,\text{m}^3\)).
On those levels, the quantity
\[
\text{pct\_err}_{0.1\text{m}}(h)
\approx 100 \times \frac{|A(h)| \times 0.10}{V(h)}
\]
gives the **percent error in volume** associated with a \(\pm 0.10\) m uncertainty
in water level at stage \(h\).
